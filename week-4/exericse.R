...
highest_rebounds <- filter(team.data, TRB == max(TRB)) %>% 
  select(Team, TRB)
...
# within each group.
grouped.water.pkm.generation <- grouped.water.pkm %>% 
  summarize("Mean speed" = mean(Speed), "Max HP" = max(HP))
...
pokemon.specific <- pokemon %>% 
  filter(Legendary == "False") %>% 
  group_by(Generation) %>% 
  summarize("Maximum attack" = max(Attack), "Minimum attack" = min(Attack), "Maximum defense" = max(Defense), "Minimum defense" = min(Defense))
